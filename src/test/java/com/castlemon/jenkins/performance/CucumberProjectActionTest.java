...
public class CucumberProjectActionTest {
    ...
    public void checkConstructorNullSummary() throws IOException {
        FileUtils.forceDelete(testFolder.newFile("cucumber-perf-reports/cukeperf.xml"));
        cucumberProjectAction = new CucumberProjectAction(project, 20);
    ...
    public void testGetFeature() throws IOException {
        List<Summary> summaryList = new ArrayList<Summary>(cucumberProjectAction.getProjectSummary().getFeatureSummaries().values());
        Summary feature = cucumberProjectAction.getFeature(summaryList.get(0).getPageLink());
        Assert.assertEquals(summaryList.get(0).getName(), feature.getName());
    }

    @Test
    public void testGetScenario() throws IOException {
        List<Summary> summaryList = new ArrayList<Summary>(cucumberProjectAction.getProjectSummary().getScenarioSummaries().values());
        Summary scenario = cucumberProjectAction
                .getScenario(summaryList.get(0).getPageLink());
        Assert.assertEquals(summaryList.get(0).getName(), scenario.getName());
    }

    @Test
    public void testGetStep() throws IOException {
        List<Summary> summaryList = new ArrayList<Summary>(cucumberProjectAction.getProjectSummary().getStepSummaries().values());
        Summary scenario = cucumberProjectAction
    ...
    public void testGetPieChartData() throws IOException {
        ...
        //Summary scenario = cucumberProjectAction
        //        .getStep(summaryList.get(0).getPageLink());
        String pieChartData = cucumberProjectAction.getProjectSummary().getOverallSummary().getPieChartData();
        Assert.assertEquals(pieChartData, cucumberProjectAction.getPieChartData());
    }
