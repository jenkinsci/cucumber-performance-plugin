...
@PrepareForTest({ AbstractBuild.class, Computer.class, RunMap.class, AbstractProject.class })
...
public class CucumberPerfRecorderTest {
    ...
    @Test
    public void testPerformOnlyOldFiles() throws IOException, InterruptedException {
        AbstractBuild build = PowerMockito.mock(AbstractBuild.class);
        AbstractProject powerProject = PowerMockito.mock(AbstractProject.class);
        ...
        PowerMockito.when(Computer.currentComputer()).thenReturn(null); //not a slave
        PowerMockito.when(build.getProject()).thenReturn(powerProject);
        PowerMockito.when(build.getWorkspace()).thenReturn(new FilePath(testFolder.getRoot()));
        RunMap<?> runMap = Mockito.mock(RunMap.class);
        Iterator iterator = Mockito.mock(Iterator.class);
        Mockito.when(runMap.iterator()).thenReturn(iterator);
        //return true the first time, false the second
    ...
    @Test
    public void testPerformSlaveBuild() throws IOException, InterruptedException {
        AbstractBuild build = PowerMockito.mock(AbstractBuild.class);
        AbstractProject powerProject = PowerMockito.mock(AbstractProject.class);
        PowerMockito.when(powerProject.getLastCompletedBuild()).thenReturn(run);
        PowerMockito.when(powerProject.getName()).thenReturn("TestP");
        PowerMockito.mockStatic(Computer.class);
        SlaveComputer slaveComputer = Mockito.mock(SlaveComputer.class);
        PowerMockito.when(Computer.currentComputer()).thenReturn(slaveComputer);
        PowerMockito.when(build.getProject()).thenReturn(powerProject);
        PowerMockito.when(powerProject.getSomeWorkspace()).thenReturn(new FilePath(testFolder.getRoot()));
        PowerMockito.when(build.getWorkspace()).thenReturn(new FilePath(testFolder.getRoot()));
        RunMap<?> runMap = Mockito.mock(RunMap.class);
        Iterator iterator = Mockito.mock(Iterator.class);
        Mockito.when(runMap.iterator()).thenReturn(iterator);
    ...
    @Test
    public void testPerformOldAndNewFiles() throws IOException, InterruptedException {
        AbstractBuild build = PowerMockito.mock(AbstractBuild.class);
        AbstractProject powerProject = PowerMockito.mock(AbstractProject.class);
        PowerMockito.when(powerProject.getLastCompletedBuild()).thenReturn(run);
        PowerMockito.when(powerProject.getName()).thenReturn("TestP");
